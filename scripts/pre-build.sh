...
step 2 "Deploy QuietFillAuction contract"
cd "$PROJECT_DIR/tools"
: > "$LOG_FILE"  # truncate log file
DEPLOY_OUT=$(go run ./cmd/deploy-contract -a "$ADDRESSES_FILE" -c "$CHAIN_URL" 2>"$LOG_FILE") || {
    echo -e "${RED}Deploy failed. Logs:${NC}" >&2
    cat "$LOG_FILE" >&2
    die "Deploy failed — see output above"
}
INSTRUCTION_SENDER=$(echo "$DEPLOY_OUT" | tail -1)
BASE_TOKEN=$(echo "$DEPLOY_OUT" | grep '^BASE_TOKEN=' | cut -d= -f2 || true)
QUOTE_TOKEN=$(echo "$DEPLOY_OUT" | grep '^QUOTE_TOKEN=' | cut -d= -f2 || true)
